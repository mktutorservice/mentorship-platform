...
export default function PrivateRoomsPage() {
  ...
  useEffect(() => {
    async function checkAuthAndFetchStudents() {
      // 1. Verify Authentication against Supabase Session
    ...
    return (
      <div className="min-h-screen bg-[#0d0d14] flex flex-col items-center justify-between p-4">
        ...
        <div className="w-full max-w-6xl flex-1 my-4 grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="bg-[#1b1b26] rounded-2xl border border-white/10 flex flex-col items-center justify-center relative overflow-hidden shadow-2xl p-6">
            <div className="w-20 h-20 rounded-full bg-[#8458B3]/30 border-2 border-[#8458B3] flex items-center justify-center text-2xl font-bold text-white mb-2">
              {(activeCallStudent.name || activeCallStudent.username || 'S')[0].toUpperCase()}
            </div>
        ...
        <div className="w-full max-w-md bg-[#1b1b26] border border-white/10 rounded-2xl p-3 flex items-center justify-around">
          <button className="px-4 py-2 bg-gray-800 hover:bg-gray-700 rounded-xl text-white text-xs font-medium transition">
            🎤 Mute
          </button>
          <button className="px-4 py-2 bg-gray-800 hover:bg-gray-700 rounded-xl text-white text-xs font-medium transition">
            📷 Video Off
          </button>
          <button className="px-4 py-2 bg-[#8458B3] hover:bg-[#a280d3] rounded-xl text-white text-xs font-medium transition">
            🖥️ Share Screen
          </button>
        </div>
      </div>
              ...
              >
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-[#D0BDF4]">@{student.username || 'student'}</p>
                ...
                <div className="mt-6 pt-4 border-t border-white/5 grid grid-cols-3 gap-2">
                  <button
                    ...
                    className="py-2 px-3 bg-green-600 hover:bg-green-500 text-white rounded-xl text-xs font-medium transition text-center cursor-pointer"
                  >
                    📞 Start Call
                  </button>
                  <button
                    onClick={() => handleSchedule(student)}
                    className="py-2 px-3 bg-[#8458B3] hover:bg-[#a280d3] text-white rounded-xl text-xs font-medium transition text-center cursor-pointer"
                  >
                    📅 Schedule
                  </button>
                  <button
                    onClick={() => handleMessage(student)}
                    className="py-2 px-3 bg-gray-800 hover:bg-gray-700 text-white rounded-xl text-xs font-medium transition text-center cursor-pointer"
                  >
                    💬 Message
                  </button>
                </div>
